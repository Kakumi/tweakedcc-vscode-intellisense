...
export function activate(context: vscode.ExtensionContext) {
  ...
  const objets = modules.map((x) => x.getBaseCompletion());
  const providers = modules.flatMap((x) => x.getProviders());

  const objectsProviders = vscode.languages.registerCompletionItemProvider(
...
function startLanguageServer(context: vscode.ExtensionContext) {
  ...
  const serverCommand = vscode.workspace
    .getConfiguration()
    .get("lua.server.command") as string;
  if (serverCommand) {
    const serverArgs = vscode.workspace
      .getConfiguration()
      .get("lua.server.args") as string[];
    serverOptions = {
...
function createUploadPastebinCommand(context: vscode.ExtensionContext) {
  return vscode.commands.registerCommand(
    "tweakedcc-intellisense.uploadToPastebin",
    async function () {
      ...
      if (!editor) {
        vscode.window.showErrorMessage(
          "you need to open a lua file in the editor"
        );
        return;
      }
      ...
      try {
        ...
        if (action === "Copy") {
          ...
          vscode.window.showInformationMessage(
            "Pastebin URL copied to clipboard."
          );
        }
      } catch (error) {
        console.error(error);
...
async function getApiKey(secretStorage) {
  ...
  if (!apiKey) {
    ...
    if (apiKey) {
      ...
      vscode.window.showErrorMessage(
        "Pastebin API Key required to upload on Pastebin."
      );
      return null;
    }
  }
