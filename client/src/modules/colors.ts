...
export const colorsObject = new vscode.CompletionItem('colors', vscode.CompletionItemKind.Module);
colorsObject.documentation = new vscode.MarkdownString(`Constants and functions for colour values, suitable for working with term and redstone.`);
...
const colorsMethods = [
    {
        ...
        label: 'combine',
        documentation: new vscode.MarkdownString(`Combines a set of colors (or sets of colors) into a larger set. Useful for Bundled Cables.
        ...
        **Usage:** \`colors.combine(colors.white, colors.magenta, colours.lightBlue)\``),
        ...
        label: 'substract',
        documentation: new vscode.MarkdownString(`Removes one or more colors (or sets of colors) from an initial set. Useful for Bundled Cables.
        ...
        **Usage:** \`colours.subtract(colours.lime, colours.orange, colours.white)\``),
        ...
        label: 'test',
        documentation: new vscode.MarkdownString(`Tests whether color is contained within colors. Useful for Bundled Cables.
        ...
        **Usage:** \`colors.test(colors.combine(colors.white, colors.magenta, colours.lightBlue), colors.lightBlue)\``),
        ...
        label: 'packRGB',
        documentation: new vscode.MarkdownString(`Combine a three-colour RGB value into one hexadecimal representation.
        ...
        **Usage:** \`colors.packRGB(0.7, 0.2, 0.6)\``),
        ...
        label: 'unpackRGB',
        documentation: new vscode.MarkdownString(`Separate a hexadecimal RGB colour into its three constituent channels.
        ...
        **Usage:** \`colors.unpackRGB(0xb23399)\``),
        ...
        label: 'toBlit',
        documentation: new vscode.MarkdownString(`Converts the given color to a paint/blit hex character (0-9a-f).
        ...
        **Usage:** \`colors.toBlit(colors.red)\``),
        ...
        label: 'fromBlit',
        documentation: new vscode.MarkdownString(`Converts the given paint/blit hex character (0-9a-f) to a color.
...
export function getColorsProviders() {
    const provider1 = vscode.languages.registerCompletionItemProvider(
        { language: 'lua', scheme: 'file' },
        {
            provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
                ...
                // Show methods only if the line starts with 'colors.'
                if (!linePrefix.endsWith('colors.')) {
                    return undefined;
                }

                const completionItems = colorsMethods.map(method => {
    ...
    const provider2 = vscode.languages.registerSignatureHelpProvider(
        { language: 'lua', scheme: 'file' },
        {
            provideSignatureHelp(document: vscode.TextDocument, position: vscode.Position) {
                ...
                // Check if we are inside a colors method call
                const methodMatch = linePrefix.match(/colors\.(\w+)\(/);
                if (!methodMatch) {
                    return undefined;
                }
    
                const methodName = methodMatch[1];
                const method = colorsMethods.find(m => m.label === methodName);
