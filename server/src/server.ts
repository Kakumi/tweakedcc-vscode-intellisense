...
class ServiceDispatcher {
  ...
  private onDocumentSymbol(handler: DocumentSymbolParams): SymbolInformation[] {
      ...
      const analysis: Analysis.Analysis = this.perDocumentAnalysis.get(uri)!;
  ...
  private onDidClose(change: TextDocumentChangeEvent<TextDocument>) {
      this.connection.sendDiagnostics({
          uri: change.document.uri,
          diagnostics: []
      });
  ...
  private async parseAndLintDocument(document: TextDocument) {
      ...
      const parseDocument = (): Promise<Diagnostic[]> => {
          return new Promise((resolve) => {
              try {
                  this.perDocumentAnalysis.set(documentUri, new Analysis.Analysis());
                  this.perDocumentAnalysis.get(documentUri)!.end(documentText);
                  this.perDocumentAnalysis.get(documentUri)!.buildGlobalSymbols();
